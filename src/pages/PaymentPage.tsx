...
function stroopsToXLM(stroops: bigint): string {
  ...
  // Show max 7 decimals but strip trailing zeros
...
//
// NOTE: We implement the simulation here directly rather than calling
// getPaymentLink() from contract.ts because that helper uses a 55-char dummy
// account key that fails stellar-sdk v13's stricter address validation.
// The Stellar null address below is exactly 56 chars and always accepted.
//
const CONTRACT_ID_VAL = import.meta.env.VITE_CONTRACT_ID || '';
const RPC_URL_VAL = 'https://soroban-testnet.stellar.org';
// Standard Stellar null/zero account — 56 chars, valid for simulation reads
...
async function fetchLinkData(slug: string): Promise<PaymentLinkData | null> {
  ...
  // Address may be returned as StellarSdk.Address or a plain string
...
// ─── Payment sender (uses StellarWalletsKit for multi-wallet support) ─────────
...
export const PaymentPage: React.FC = () => {
  ...
  }, [isConnected, clearWalletError, handlePay]);
  ...
  const handleCopyDetails = useCallback(async () => {
    ...
    const amountXLM = stroopsToXLM(linkData.amount);
    const text =
      `Send ${amountXLM} XLM to ${linkData.merchant}\nMemo: StellarPay:${linkData.slug}`;
        ...
        >
          ...
          {pageState === 'not_found' && (
            ...
            >
              ...
              <Link to="/" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '10px 20px', borderRadius: '10px', background: 'rgba(99,102,241,0.12)', border: '1px solid rgba(99,102,241,0.25)', color: '#818cf8', fontSize: '13px', fontWeight: 500, textDecoration: 'none', transition: 'all 0.15s' }}>
                Go to StellarPay →
              </Link>
            </div>
          )}
          ...
          {pageState === 'error' && (
            ...
            >
              ...
              <button
                onClick={() => { setPageState('loading'); setFetchError(null); fetchLinkData(slug).then(d => { if (!d) setPageState('not_found'); else if (!d.active) { setLinkData(d); setPageState('inactive'); } else { setLinkData(d); setPageState('ready'); } }).catch(e => { setFetchError(e instanceof Error ? e.message : String(e)); setPageState('error'); }); }}
          ...
          {/* ══════════════════════════════════════════
              PAYMENT SUCCESS STATE (Option A)
          ══════════════════════════════════════════ */}
          {pageState === 'ready' && payStep === 'success' && linkData && (
            <div className="sp-card pp-fade-up" style={{ textAlign: 'center', padding: '40px 24px' }}>
                ...
                {txHash && (
                  <div>
                    ...
                    <a
                      ...
                      style={{ fontSize: '11px', color: '#6366f1', fontFamily: 'monospace', textDecoration: 'underline', wordBreak: 'break-all' }}
                    >
                      {truncAddr(txHash, 10)} ↗
                    </a>
                  </div>
              ...
              <Link
                to={`/pay/${slug}`}
                onClick={() => { setPayStep('idle'); setTxHash(null); setPayError(null); }}
                style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '10px 20px', borderRadius: '10px', background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.2)', color: '#818cf8', fontSize: '13px', fontWeight: 500, textDecoration: 'none' }}
              >
                Return to sender's page
              </Link>
            </div>
          )}
          ...
          {pageState === 'ready' && payStep !== 'success' && linkData && (
            <div className="sp-card animate-slide-up">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', paddingBottom: '20px', borderBottom: '1px solid var(--surface-border)', marginBottom: '20px' }}>
                ...
                <p style={{ margin: 0, fontSize: '12px', color: 'var(--text-tertiary)', letterSpacing: '0.03em' }}>
              ...
              >
                <div>
                  <p style={{ margin: '0 0 4px 0', fontSize: '10px', fontWeight: 600, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                    Amount
                  </p>
                  <p style={{ margin: 0, fontSize: '28px', fontWeight: 700, color: 'var(--text-primary)', letterSpacing: '-0.03em', lineHeight: 1.1 }}>
                    {amountXLM}{' '}
                ...
                <div>
                  <p style={{ margin: '0 0 2px 0', fontSize: '10px', fontWeight: 600, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                    For
                  </p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                {/* Option A — Connect wallet and pay */}
                <button
                  id="pay-wallet-btn"
                  onClick={handleConnectOrPay}
                  disabled={payStep === 'paying'}
                  className="btn-cta"
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', fontSize: '14px' }}
                >
                  {payStep === 'paying' ? (
                    <>
                ...
                {payStep === 'failed' && payError && (
                  <div
                    ...
                    style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', padding: '10px 12px', borderRadius: '10px', background: 'rgba(239,68,68,0.07)', border: '1px solid rgba(239,68,68,0.2)' }}
                  >
                    ...
                    <p style={{ margin: 0, fontSize: '12px', color: '#ef4444', flex: 1, lineHeight: 1.5 }}>
                      {payError}
                    </p>
                    <button
                      onClick={() => { setPayStep('idle'); setPayError(null); }}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444', opacity: 0.6, flexShrink: 0, padding: '2px' }}
                    >
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      <div style={{ padding: '10px 12px', borderRadius: '8px', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.06)' }}>
                        ...
                        <p style={{ margin: 0, fontSize: '11px', color: 'var(--text-secondary)', fontFamily: 'monospace', wordBreak: 'break-all' }}>
                          {linkData.merchant}
                        </p>
                      </div>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <div style={{ flex: 1, padding: '10px 12px', borderRadius: '8px', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.06)' }}>
                        ...
                        <div style={{ flex: 1, padding: '10px 12px', borderRadius: '8px', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.06)' }}>
                          ...
                          <p style={{ margin: 0, fontSize: '11px', color: 'var(--text-secondary)', fontFamily: 'monospace', wordBreak: 'break-all' }}>
                            StellarPay:{linkData.slug}
                          </p>
                        </div>
                      </div>
                    </div>

                    {/* Optional: paste your address */}
                    <div>
                      <label
                        htmlFor="manual-addr-input"
                        style={{ display: 'block', fontSize: '11px', color: 'var(--text-tertiary)', marginBottom: '6px' }}
                      >
                      ...
                      />
                      {manualAddrError && (
                        <p style={{ margin: '4px 0 0 0', fontSize: '11px', color: 'var(--error)' }}>
                          {manualAddrError}
                        </p>
                      )}
                    ...
                    <button
                      ...
                      style={{
                        ...
                        height: '38px',
                        borderRadius: '8px',
                        background: detailsCopied ? 'rgba(34,197,94,0.12)' : 'rgba(255,255,255,0.04)',
                        border: `1px solid ${detailsCopied ? 'rgba(34,197,94,0.3)' : 'rgba(255,255,255,0.1)'}`,
                        color: detailsCopied ? '#22c55e' : 'var(--text-secondary)',
                        fontSize: '12px',
                        fontWeight: 500,
                        cursor: 'pointer',
